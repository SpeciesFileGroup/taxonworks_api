...
mod(CodeMirror)})(function(CodeMirror){"use strict";function forEach(arr,f){for(var i=0;i<arr.length;i++){f(arr[i],i)}}function some(arr,f){for(var i=0;i<arr.length;i++){if(f(arr[i],i))return!0}return(/* eat */!1)}CodeMirror.defineMode("dylan",function(_config){// Words
var words={// Words that introduce unnamed definitions like "define interface"
...
var symbolPattern="[-_a-zA-Z?!*@<>$%]+",symbol=/^[-_a-zA-Z?!*@<>$%]+/,patterns={// Symbols with special syntax
symbolKeyword:symbolPattern+":",symbolClass:"<"+symbolPattern+">",symbolGlobal:"\\*"+symbolPattern+"\\*",symbolConstant:"\\$"+symbolPattern},patternStyles={symbolKeyword:"atom",symbolClass:"tag",symbolGlobal:"variable-2",symbolConstant:"variable-3"};// Compile all patterns to regular expressions
...
var ch=stream.peek();if("'"==ch||"\""==ch){stream.next();return chain(stream,state,tokenString(ch,"string"))}// Comment
else if("/"==ch){stream.next();if(stream.eat("*")){return chain(stream,state,tokenComment)}else if(stream.eat("/")){stream.skipToEnd();return"comment"}stream.backUp(1)}// Decimal
else if(/[+\-\d\.]/.test(ch)){if(stream.match(/^[+-]?[0-9]*\.[0-9]*([esdx][+-]?[0-9]+)?/i)||stream.match(/^[+-]?[0-9]+([esdx][+-]?[0-9]+)/i)||stream.match(/^[+-]?\d+/)){return"number"}}// Hash
else if("#"==ch){stream.next();// Symbol with string syntax
ch=stream.peek();if("\""==ch){stream.next();return chain(stream,state,tokenString("\"","string"))}// Binary number
else if("b"==ch){stream.next();stream.eatWhile(/[01]/);return"number"}// Hex number
else if("x"==ch){stream.next();stream.eatWhile(/[\da-f]/i);return"number"}// Octal number
else if("o"==ch){stream.next();stream.eatWhile(/[0-7]/);return"number"}// Token concatenation in macros
else if("#"==ch){stream.next();return"punctuation"}// Sequence literals
else if("["==ch||"("==ch){stream.next();return"bracket";// Hash symbol
}else if(stream.match(/f|t|all-keys|include|key|next|rest/i)){return"atom"}else{stream.eatWhile(/[-a-zA-Z]/);return"error"}}else if("~"==ch){stream.next();ch=stream.peek();if("="==ch){stream.next();ch=stream.peek();if("="==ch){stream.next();return"operator"}return"operator"}return"operator"}else if(":"==ch){stream.next();ch=stream.peek();if("="==ch){stream.next();return"operator"}else if(":"==ch){stream.next();return"punctuation"}}else if(-1!="[](){}".indexOf(ch)){stream.next();return"bracket"}else if(-1!=".,".indexOf(ch)){stream.next();return"punctuation"}else if(stream.match("end")){return"keyword"}for(var name in patterns){if(patterns.hasOwnProperty(name)){var pattern=patterns[name];if(babelHelpers.instanceof(pattern,Array)&&some(pattern,function(p){return stream.match(p)})||stream.match(pattern))return patternStyles[name]}}if(/[+\-*\/^=<>&|]/.test(ch)){stream.next();return"operator"}if(stream.match("define")){return"def"}else{stream.eatWhile(/[\w\-]/);// Keyword
if(wordLookup.hasOwnProperty(stream.current())){return styleLookup[stream.current()]}else if(stream.current().match(symbol)){return"variable"}else{stream.next();return"variable-2"}}}function tokenComment(stream,state){var maybeEnd=!1,maybeNested=!1,nestedCount=0,ch;while(ch=stream.next()){if("/"==ch&&maybeEnd){if(0<nestedCount){nestedCount--}else{state.tokenize=tokenBase;break}}else if("*"==ch&&maybeNested){nestedCount++}maybeEnd="*"==ch;maybeNested="/"==ch}return"comment"}function tokenString(quote,style){return function(stream,state){var escaped=!1,next,end=!1;while(null!=(next=stream.next())){if(next==quote&&!escaped){end=!0;break}escaped=!escaped&&"\\"==next}if(end||!escaped){state.tokenize=tokenBase}return style}}// Interface
return{startState:function startState(){return{tokenize:tokenBase,currentIndent:0}},token:function token(stream,state){if(stream.eatSpace())return null;var style=state.tokenize(stream,state);return style},blockCommentStart:"/*",blockCommentEnd:"*/"}});CodeMirror.defineMIME("text/x-dylan","dylan")});